...
if input_train.uns["dataset_organism"] != "homo_sapiens":
    exit_non_applicable(
        f"TranscriptFormer (MLflow) can only be used with human data "
...
def process_transcriptformer_adata(adata):
    """Add assay column if missing (TranscriptFormer requires it)."""
    if "assay" not in adata.obs or adata.obs["assay"].isna().all():
...
predictions = embed_and_classify(
    ...
    process_adata=process_transcriptformer_adata,
)
